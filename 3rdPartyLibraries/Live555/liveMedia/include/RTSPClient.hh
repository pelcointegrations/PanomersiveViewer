...
public:
  ...
  __declspec(dllexport) unsigned sendDescribeCommand(responseHandler* responseHandler, Authenticator* authenticator = NULL);
  ...
  __declspec(dllexport) unsigned sendSetupCommand(MediaSubsession& subsession, responseHandler* responseHandler,
  ...
  __declspec(dllexport) unsigned sendPlayCommand(MediaSession& session, responseHandler* responseHandler,
      ...
      // (The "responseHandler" and "authenticator" parameters are as described for "sendDescribeCommand".)
  __declspec(dllexport) unsigned sendPlayCommand(MediaSubsession& subsession, responseHandler* responseHandler,
			   double start = 0.0f, double end = -1.0f, float scale = 1.0f,
			   Authenticator* authenticator = NULL);
  ...
  __declspec(dllexport) unsigned sendTeardownCommand(MediaSession& session, responseHandler* responseHandler, Authenticator* authenticator = NULL);
      // Issues an aggregate RTSP "TEARDOWN" command on "session", then returns the "CSeq" sequence number that was used in the command.
      // (The "responseHandler" and "authenticator" parameters are as described for "sendDescribeCommand".)
  __declspec(dllexport) unsigned sendTeardownCommand(MediaSubsession& subsession, responseHandler* responseHandler, Authenticator* authenticator = NULL);
...
protected:
    __declspec(dllexport) RTSPClient(UsageEnvironment& env, char const* rtspURL,
	     int verbosityLevel, char const* applicationName, portNumBits tunnelOverHTTPPortNum, int socketNumToServer);
      // called only by createNew();
    __declspec(dllexport) virtual ~RTSPClient();

  void reset();
  void setBaseURL(char const* url);
  int grabSocket(); // allows a subclass to reuse our input socket, so that it won't get closed when we're deleted
  __declspec(dllexport) virtual unsigned sendRequest(RequestRecord* request);
  __declspec(dllexport) virtual Boolean setRequestFields(RequestRecord* request,
...
private: // redefined virtual functions
    __declspec(dllexport) virtual Boolean isRTSPClient() const;
