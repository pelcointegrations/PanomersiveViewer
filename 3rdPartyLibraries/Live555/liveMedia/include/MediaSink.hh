...
public:
  ...
  __declspec(dllexport) Boolean startPlaying(MediaSource& source,
		       afterPlayingFunc* afterFunc,
		       void* afterClientData);
  __declspec(dllexport) virtual void stopPlaying();

  // Test for specific types of sink:
  __declspec(dllexport) virtual Boolean isRTPSink() const;

  FramedSource* source() const {return fSource;}

protected:
  __declspec(dllexport)  MediaSink(UsageEnvironment& env); // abstract base class
  __declspec(dllexport) virtual ~MediaSink();

  __declspec(dllexport) virtual Boolean sourceIsCompatibleWithUs(MediaSource& source);
      // called by startPlaying()
  virtual Boolean continuePlaying() = 0;
      // called by startPlaying()

  __declspec(dllexport) static void onSourceClosure(void* clientData); // can be used in "getNextFrame()" calls
