...
public:
    __declspec(dllexport) static MediaSession* createNew(UsageEnvironment& env,
  ...
  __declspec(dllexport) char* absStartTime() const;
  __declspec(dllexport) char* absEndTime() const;
  // Used only to set the local fields:
  __declspec(dllexport) char*& _absStartTime() { return fAbsStartTime; }
  __declspec(dllexport) char*& _absEndTime() { return fAbsEndTime; }
...
public:
    __declspec(dllexport) MediaSubsessionIterator(MediaSession const& session);
    __declspec(dllexport) virtual ~MediaSubsessionIterator();

  __declspec(dllexport) MediaSubsession* next(); // NULL if none
  void reset();
...
public:
  ...
  __declspec(dllexport) Boolean initiate(int useSpecialRTPoffset = -1);
