...
public:
  ...
  __declspec(dllexport) virtual char const* sdpMediaType() const; // for use in SDP m= lines
  __declspec(dllexport) virtual char* rtpmapLine() const; // returns a string to be delete[]d
  __declspec(dllexport) virtual char const* auxSDPLine();
...
protected:
    __declspec(dllexport) RTPSink(UsageEnvironment& env,
    ...
    __declspec(dllexport) virtual ~RTPSink();
